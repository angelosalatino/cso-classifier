...
#    rt = clf.classify(paper, num_narrower=1, min_similarity=0.94, climb_ont='wt', verbose=False)
        ...
        #args=(paper, 1, 1, 'wt', False, result["stringmatcher"])

        p1 = mp.Process(target=self.classify, args=[self.paper, shared_dict, 1, 1, 'wt', False])
        processes.append(p1)
        
        p2 = mp.Process(target=self.classify, args=[self.paper, shared_dict,1, 0.94, 'wt', False])
        processes.append(p2)
    ...
    def classify(self, paper, shared_dict, num_narrower=2, min_similarity=0.85, climb_ont='jfb', verbose=False):
        topics = self.classify(paper, num_narrower, min_similarity, climb_ont, verbose)
        
        if(min_similarity == 1):
            shared_dict["stringsimilarity"] = topics
        else:
            shared_dict["stringmatcher"] = topics

    def classify2(self, paper, num_narrower=2, min_similarity=0.85, climb_ont='jfb', verbose=False):
        ...
        topics['extracted'] = extracted_topics
        ...
        if(num_narrower > 0):
            ...
            topics['inferred'] = inferred_topics
